#!/usr/bin/env python3
"""Verify every downloaded shapefile zip with ogrinfo (GDAL /vsizip/).
Reports geometry type, feature count, and CRS, and cross-checks the count
against the WFS feature count recorded in manifest.json. Flags empty,
mismatched (truncated), or unreadable layers -> verification.csv."""
...
def ogr_summary(zippath):
    """Return (shp, geomtype, nfeatures, epsg, note) using ogrinfo -so."""
...
for z in zips:
    ...
    print(f"  {fname:40} {str(geom):14} {str(nf):>8} feats  EPSG:{str(crs):6} {match:12} {flag}")
...
for label, lst in [("BAD", bad), ("EMPTY", empty), ("MISMATCH", mism)]:
    for r in lst:
        print(f"   {label}: {r['file']} ({r['note']}, wfs={r['wfs_features']}, got={r['features']})")
